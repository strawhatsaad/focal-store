...
const DONATION_PRODUCT_VARIANT_ID =
...
function CartPageContent() {
  ...
  const uiCalculatedSubtotal = lineItemsWithDisplayPrice.reduce((acc, item) => {
  ...
  const discountAmount = isFirstTimeCustomer ? uiCalculatedSubtotal * 0.2 : 0;
          ...
          <div className="bg-white p-3 xs:p-4 sm:p-6 md:p-8 rounded-xl shadow-xl">
            <ul role="list" className="divide-y divide-gray-200">
              {lineItemsWithDisplayPrice.map((line) => {
                const isDonation =
                  line.merchandise.id === DONATION_PRODUCT_VARIANT_ID;
                const itemDiscount =
                  isFirstTimeCustomer && !isDonation
                    ? line.displayTotalPrice * 0.2
                    : 0;
                    ...
                    <div className="ml-0 md:ml-4 mt-3 md:mt-0 flex-1 flex flex-col">
                      <div>
                        ...
                        {line.attributes && line.attributes.length > 0 && (
                          <div className="mt-2 text-sm text-gray-600 w-full md:max-w-md">
                            {line.isCustomizedEyeglass ? (
                                ...
                                {expandedLines[line.id] && (
                                  <div className="p-4 bg-white space-y-3 border-t border-gray-100">
                                    {line.attributes.some(
                                      (attr) =>
                                        attr.key === "Rx Method" &&
                                        attr.value === "Will Provide Later",
                                    ) && (
                                      <div className="mb-4 p-3 bg-red-50 border border-red-100 rounded-md flex justify-between items-center">
                                        <p className="text-xs text-red-800 font-medium">
                                          Prescription Needed
                                        </p>
                                        <Link
                                          href={`/products/eyewear/${line.merchandise.product.handle}`}
                                          className="text-xs bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-full font-medium transition-colors"
                                        >
                                          Upload
                                        </Link>
                                      </div>
                                    ...
                                    {line.attributes
                                      .filter(
                                        (attr) =>
                                          attr.key !==
                                            "_finalCalculatedPrice" &&
                                          attr.key !== "FocalProductType" &&
                                          attr.key !== "Frame",
                                      )
                                      .map((attr) => (
                                        <div
                                          key={attr.key}
                                          className="flex justify-between text-[13px] items-start gap-4"
                                        >
                                    ...
                                    <div className="pt-3 mt-3 border-t border-gray-100">
                                      <Link
                                        href={`/products/eyewear/${line.merchandise.product.handle}?edit=${line.id}`}
                                        className="inline-flex items-center text-xs font-medium text-black hover:text-gray-600 underline underline-offset-2"
                                      >
                              ...
                              <div className="mt-1 space-y-0.5">
                                {line.attributes
                                  .filter(
                                    (attr) =>
                                      attr.key !== "_finalCalculatedPrice",
                                  )
                                  .map((attr) => {
                                    if (attr.key === "Donation Message") {
                                      return (
                                        <p
                                          key={attr.key}
                                          className="text-xs text-gray-500 max-w-md rounded-md mt-1"
                                        >
                                          {attr.value}
                                        </p>
                                      );
                ...
                {isFirstTimeCustomer && (
                  <div className="flex justify-between text-green-600 font-medium">
                    <p className="flex items-center gap-1">
                      <Tag size={14} /> First-Time Discount (20%)
                    </p>
