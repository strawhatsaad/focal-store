...
export async function storeFront(
  ...
  if (!apiURL) {
    console.error(
      "Shopify Storefront API URL is not defined in environment variables (NEXT_PUBLIC_API_URL)."
    );
    throw new Error("Shopify Storefront API URL is not configured.");
  }
  if (!process.env.NEXT_PUBLIC_STOREFRONT_ACCESS_TOKEN) {
    console.error(
      "Shopify Storefront Access Token is not defined in environment variables (NEXT_PUBLIC_STOREFRONT_ACCESS_TOKEN)."
    );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[storeFront] Shopify API HTTP Error (${response.status}): ${errorBody}`
      );
      ...
      try {
        ...
        if (jsonError && jsonError.errors && Array.isArray(jsonError.errors)) {
          errorMessage += ` - Details: ${jsonError.errors
            .map((e) => e.message || "Unknown Shopify error")
            .join(", ")}`;
        } else if (jsonError && jsonError.error) {
          errorMessage += ` - Details: ${jsonError.error}`;
        }
      } catch (e) {
        errorMessage += ` - Raw Response Body: ${errorBody.substring(0, 300)}${
    ...
    if (
      ...
      const errorMessages = result.errors
        .map((e) => e.message || "Unknown Shopify GraphQL error")
        .join("; ");
      console.error(
        "[storeFront] Shopify GraphQL Errors:",
    ...
    console.error(
      "[storeFront] Error during Shopify API call execution:",
      error.message
    );
    if (error.message.toLowerCase().includes("failed to fetch")) {
      throw new Error(
        `Network error or CORS issue: Failed to fetch from ${apiURL}. Check browser console. Original error: ${error.message}`
      );
    }
    throw error;
  }
...
export async function shopifyAdminRequest(query, variables = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      console.error(
        `[shopifyAdminRequest] Shopify Admin API HTTP Error (${response.status}) for URL ${adminApiURL}: ${errorBody}`
      );
      ...
      try {
        ...
        if (jsonError && jsonError.errors) {
          if (Array.isArray(jsonError.errors)) {
            errorMessage += ` - Details: ${jsonError.errors
              .map((e) => e.message || "Unknown Shopify Admin error")
              .join(", ")}`;
    ...
    if (
      ...
      const errorMessages = result.errors
        .map((e) => e.message || "Unknown Shopify Admin GraphQL error")
        .join("; ");
      console.error(
        "[shopifyAdminRequest] Shopify Admin GraphQL Errors:",
        JSON.stringify(result.errors, null, 2)
      );
    ...
    if (mutationName && result.data?.[mutationName]?.userErrors?.length > 0) {
      ...
      console.warn(
        `[shopifyAdminRequest] Shopify Admin User Errors in ${mutationName}:`,
    ...
    console.error(
      "[shopifyAdminRequest] Error during Shopify Admin API call execution:",
      error.message
    );
    if (error.message.toLowerCase().includes("failed to fetch")) {
      throw new Error(
        `Network error: Failed to fetch from ${adminApiURL}. Original error: ${error.message}`
      );
    }
...
export const GET_CUSTOMER_BY_EMAIL_QUERY = `
...
export const CREATE_CUSTOMER_ADMIN_MUTATION = `
...
export async function associateCartWithCustomer(
  ...
  if (!cartId || !customerAccessToken) {
    ...
    throw new Error(
      "Cart ID and Customer Access Token are required for association."
    );
  }
  const buyerIdentity = {
  ...
  return storeFront(CART_BUYER_IDENTITY_UPDATE_MUTATION, {
    cartId,
    buyerIdentity,
  });
}
...
export async function createShopifyDraftOrder(draftOrderInput) {
  ...
  return shopifyAdminRequest(DRAFT_ORDER_CREATE_MUTATION, {
...
export async function getShopifyCustomerMetafield(customerId, namespace, key) {
  if (!customerId || !namespace || !key) {
    throw new Error(
      "Customer ID, namespace, and key are required to fetch a metafield."
    );
  }
  return shopifyAdminRequest(GET_CUSTOMER_METAFIELD_QUERY, {
    customerId,
...
export async function createStagedUploads(input) {
  if (!input || !Array.isArray(input) || input.length === 0) {
    throw new Error(
      "Input for createStagedUploads is required and must be an array."
    );
  }
...
export async function createShopifyFiles(filesInput) {
  if (!filesInput || !Array.isArray(filesInput) || filesInput.length === 0) {
    throw new Error(
      "Input for createShopifyFiles is required and must be an array."
    );
  }
